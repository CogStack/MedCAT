    ...
    def encode(self, examples, ignore_subwords=False):
        ...
        for _ind, example in enumerate(
            ...
            tokens = self.hf_tokenizer(
                example[0], return_offsets_mapping=True, add_special_tokens=False
            )
            entities = [
                (start, end, cui)
                for start, end, cui in zip(example[1], example[2], example[3])
            ]
            ...
            for ind in range(len(tokens["offset_mapping"])):
                ...
                if entities and (
                    offset[0] >= entities[0][0] and offset[1] <= entities[0][1]
                ):
                    ...
                    if (
                        not ignore_subwords
                        or self.id2type[tokens["input_ids"][ind]] == "start"
                    ):
                    ...
                    if entities[0][1] <= offset[1]:
                        ...
                        del entities[0]
                        tkn_part_of_entity = (
                            False  # Set this so the next token is not removed
                        )

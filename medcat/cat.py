...
class CAT(object):
    ...
    def __init__(self, cdb, config, vocab, meta_cats=[]):
        ...
        self.pipe.add_tagger(
            tagger=tag_skip_and_punct,
            name="skip_and_punct",
            additional_fields=["is_punct"],
        )

        spell_checker = BasicSpellChecker(
            cdb_vocab=self.cdb.vocab, config=self.config, data_vocab=vocab
        )
    ...
    def create_model_pack(self, save_dir_path, model_pack_name="medcat_model_pack"):
        ...
        shutil.make_archive(
            os.path.join(_save_dir_path, model_pack_name), "zip", root_dir=save_dir_path
        )
    ...
    def load_model_pack(cls, zip_path):
        ...
        if os.path.exists(model_pack_path):
            print(
                "Found an existing unziped model pack at: {}, the provided zip will not be touched.".format(
                    model_pack_path
                )
            )
        else:
        ...
        cdb.config.general["spacy_model"] = os.path.join(
            model_pack_path, cdb.config.general["spacy_model"]
        )
        ...
        meta_paths = [
            os.path.join(model_pack_path, path)
            for path in os.listdir(model_pack_path)
    ...
    def _print_stats(
        ...
        for pind, project in tqdm(
            enumerate(data["projects"]),
            desc="Stats project",
            ...
            for dind, doc in tqdm(
                ...
                if use_cui_doc_limit:
                    _cuis = set(
                        [
                            ann["cui"]
                            for ann in anns
                            if check_filters(ann["cui"], _filters)
                        ]
                    )
                    if _cuis:
                        filters["cuis"] = _cuis
                ...
                for ann in anns:
                    cui = ann["cui"]
                    if (not use_filters and not use_cui_doc_limit) or check_filters(
                        cui, filters
                    ):
                        if use_groups:
                        ...
                        if ann.get("validated", True) and (
                            ...
                            and not ann.get("deleted", False)
                        ):
                            anns_norm.append((ann["start"], cui))
                            anns_examples.append(
                                {
                                    "text": doc["text"][
                                        max(0, ann["start"] - 60) : ann["end"] + 60
                                    ],
                                    "cui": cui,
                ...
                for ann in p_anns:
                    ...
                    p_anns_examples.append(
                        {
                            "text": doc["text"][
                                max(0, ann.start_char - 60) : ann.end_char + 60
                            ],
                            "cui": cui,
                ...
                for iann, ann in enumerate(anns_norm):
                    if ann not in p_anns_norm:
                        ...
                        examples["fn"][cui] = examples["fn"].get(cui, []) + [
                            anns_examples[iann]
                        ]
        ...
        try:
            ...
            print(
                "Docs with false positives: {}\n".format(
                    "; ".join([str(x) for x in list(fp_docs)[0:10]])
                )
            )
            print(
                "Docs with false negatives: {}\n".format(
                    "; ".join([str(x) for x in list(fn_docs)[0:10]])
                )
            )
            ...
            fps = {
                k: v
                for k, v in sorted(fps.items(), key=lambda item: item[1], reverse=True)
            }
            fns = {
                k: v
                for k, v in sorted(fns.items(), key=lambda item: item[1], reverse=True)
            }
            tps = {
                k: v
                for k, v in sorted(tps.items(), key=lambda item: item[1], reverse=True)
            }
            ...
            pr_fps = [
                (
                    self.cdb.cui2preferred_name.get(
                        cui, list(self.cdb.cui2names.get(cui, [cui]))[0]
                    ),
                    cui,
                    fps[cui],
                )
            ...
            for one in pr_fps:
                print(
                    "{:70} - {:20} - {:10}".format(
                        str(one[0])[0:69], str(one[1])[0:19], one[2]
                    )
                )
            print("\n\nFalse Negatives\n")
            for one in pr_fns:
    ...
    def add_and_train_concept(
        ...
        if spacy_entity is not None and spacy_doc is not None:
            # Train Linking
            self.linker.context_model.train(
                cui=cui,
                entity=spacy_entity,
                doc=spacy_doc,
                negative=negative,
            ...
            if not negative and devalue_others:
                ...
                for _cui in cuis:
                    self.linker.context_model.train(
                        cui=_cui, entity=spacy_entity, doc=spacy_doc, negative=True
                    )
        ...
        if n_process is None:
            ...
            try:
                ...
                for doc in tqdm(docs, total=len(texts)):
                    ...
                    out.append(
                        self._doc_to_out(doc, only_cui, addl_info, out_with_text=True)
                    )
                ...
                if len(out) < len(texts):
                    self.log.warning(
                        "Found at least one failed batch and set output for enclosed texts to empty"
                    )
    ...
    def _run_nn_components(self, docs, nn_components, id2text):
        ...
        for spacy_doc in spacy_docs:
            for ent in spacy_doc.ents:
                docs[spacy_doc.id]["entities"][ent._.id]["meta_anns"].update(
                    ent._.meta_anns
                )
    ...
    def _save_docs_to_file(
        ...
        part_counter = (
            part_counter + 1
        )  # Increase for save, as it should be what is the next part
    ...
    def multiprocessing(
        ...
        annotated_ids_path = (
            os.path.join(save_dir_path, "annotated_ids.pickle")
            if save_dir_path is not None
            else None
        )
        ...
        for batch in self._batch_generator(
            data, batch_size_chars, skip_ids=set(annotated_ids)
        ):
            self.log.info(
                "Annotated until now: {} docs; Current BS: {} docs".format(
                    len(annotated_ids), len(batch)
                )
            )
            try:
                _docs = self._multiprocessing_batch(
                    data=batch,
                    nproc=nproc,
                ...
                if out_split_size is not None and len(docs) > out_split_size:
                    ...
                    part_counter = self._save_docs_to_file(
                        docs=docs,
    ...
    def multiprocessing_pipe(
        ...
        n_process = (
            ...
            else min(max(cpu_count() - 1, 1), math.ceil(len(in_data) / batch_factor))
        )
        batch_size = (
            batch_size
            if batch_size is not None
            else math.ceil(len(in_data) / (batch_factor * abs(n_process)))
        )

        entities = self.get_entities_multi_texts(
            texts=in_data,
            only_cui=only_cui,
            addl_info=addl_info,
            n_process=n_process,
        ...
        while True:
            if not in_q.empty():
                ...
                for id, text in data:
                    try:
                        # Annotate document
                        doc = self.get_entities(
                            text=text, only_cui=only_cui, addl_info=addl_info
                        )
                        out.append((id, doc))
                    except Exception as e:
    ...
    def _get_trimmed_text(self, text: str) -> Optional[str]:
        return (
            text[0 : self.config.preprocessing.get("max_document_length")]
            if text is not None and len(text) > 0
            else ""
        )

    def _generate_trimmed_texts(
        self, texts: Union[Iterable[str], Iterable[Tuple]]
    ...
    def _pipe_error_handler(proc_name, proc, docs, e):
        CAT.log.warning(
            "Exception raised when applying component {} to a batch of docs.".format(
                proc_name
            )
        )

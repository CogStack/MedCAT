...
class CAT(object):
    ...
    def train(self,
        ...
        Args:
            ...
            skip_cnt (int):
                How many documents/sentences to skip from the data_iterator
        ...
        for line in data_iterator:
            if line is not None and line:
                ...
                if cnt > skip_cnt:
                    ...
                    if self.config.linking['output_dir'] is not None and cnt % self.config.linking['save_steps'] == 0:
                        # Save the CDB
                        os.makedirs(self.config.linking['output_dir'], exist_ok=True)
                        self.cdb.save(os.path.join(self.config.linking['output_dir'], 'cdb-ckpt-{}.dat'.format(cnt)))
                        self.log.info("Checkpoint saved: %s", os.path.join(self.config.linking['output_dir'],
                                                                           'cdb-ckpt-{}.dat'.format(cnt)))

...
class SpacyCat(object):
    ...
    def __init__(self, cdb, vocab=None, train=False, force_train=False, tokenizer=None):
        ...
        if tokenizer is None:
            ...
            build_vocab_from_hf(
                model_name=tokenizer, hf_tokenizer=self.hf_tokenizer, vocab=self.vocab
            )
        else:
    ...
    def _calc_acc(self, cui, doc, tkns, name=None):
        ...
        cntx = None
        cntx_short = None
        words, weights = self._get_doc_words(
            doc, tkns, span=self.CNTX_SPAN, skip_words=True, skip_current=False
        )
        words_short, weights_short = self._get_doc_words(
            doc, tkns, span=self.CNTX_SPAN_SHORT, skip_current=True
        )
        ...
        if self.DEBUG:
            if cui in self.cdb.cui2context_vec and len(cntx_vecs) > 0:
                ...
                log.debug(cui)
                log.debug(tkns)
                log.debug(np.dot(unitvec(cntx), unitvec(self.cdb.cui2context_vec[cui])))
                log.debug(":::::::::::::::::::::::::::::::::::\n")
        ...
        if cui in self.cdb.cui2context_vec and len(cntx_vecs) > 0:
            ...
            if cui in self.cdb.cui2context_vec_short and len(cntx_vecs_short) > 0:
                sim2 = np.dot(
                    unitvec(cntx_short), unitvec(self.cdb.cui2context_vec_short[cui])
                )
    ...
    def _add_cntx_vec(self, cui, doc, tkns, negative=False, lr=None, anneal=None):
        ...
        if len(cntx_vecs) > 0:
            ...
            self.cdb.add_context_vec(
                cui, cntx, cntx_type="MED", negative=negative, lr=lr, anneal=anneal
            )
        ...
        if np.random.rand() < self.NEG_PROB and not negative:
            ...
            negs = self.vocab.get_negative_samples(
                n=self.CNTX_SPAN * 2, ignore_punct_and_num=True, stopwords=STOP_WORDS
            )  # TODO stopwords
        ...
        if self.DEBUG:
            if cui in self.cdb.cui2context_vec and len(cntx_vecs) > 0:
                if np.dot(unitvec(cntx), unitvec(self.cdb.cui2context_vec[cui])) < 0.01:
                    ...
                    log.debug(tkns)
                    log.debug(
                        np.dot(unitvec(cntx), unitvec(self.cdb.cui2context_vec[cui]))
                    )
                    log.debug(":::::::::::::::::::::::::::::::::::\n")
            ...
            if cui in self.cdb.cui2context_vec_short and len(cntx_vecs_short) > 0:
                if (
                    ...
                    log.debug(
                        np.dot(
                            unitvec(cntx_short), unitvec(self.cdb.cui2context_vec[cui])
                        )
                    )
                    log.debug(":::::::::::::::::::::::::::::::::::\n")
    ...
    def _add_ann(self, cui, doc, tkns, acc=-1, name=None, is_disamb=False):
        ...
        # Skip if tui filter
        if (
            (self.TUI_FILTER is None and self.CUI_FILTER is None)
            or (
                self.TUI_FILTER
                and cui in self.cdb.cui2tui
                and self.cdb.cui2tui[cui] in self.TUI_FILTER
            )
            or (self.CUI_FILTER and cui in self.CUI_FILTER)
        ):
            if not is_disamb and cui in self.cdb.cui_disamb_always:
                ...
                if self.LBL_STYLE == "long":
                    lbl = "{} - {} - {} - {} - {:.2}".format(
                        ...
                        self.cdb.tui2name.get(
                            self.cdb.cui2tui.get(cui, "unk:unk"), "unk:unk"
                        ),
                        float(acc),
                    )
    ...
    def __call__(self, doc):
        ...
        for i in range(len(_doc)):
            ...
            if (
                raw_name in self.cdb.name2cui
                and len(raw_name) > self.MIN_CONCEPT_LENGTH
            ):
                # Add annotation
                if not self.train or not self._train_skip(raw_name) or self.force_train:
                    if not _doc[i].is_stop:
                        if (
                            self.DISAMB_EVERYTHING
                        ):  # Not needed, just move the disambiguation limit to 100
                            self.to_disamb.append((list(tkns), raw_name))
                        else:
                            self.cat_ann.add_ann(
                                raw_name, tkns, doc, self.to_disamb, doc_words
                            )
                ...
                if not self.train or not self._train_skip(name) or self.force_train:
                    if not _doc[i].is_stop:
                        if self.DISAMB_EVERYTHING:
                            ...
                            self.cat_ann.add_ann(
                                name, tkns, doc, self.to_disamb, doc_words
                            )
            ...
            for j in range(i + 1, len(_doc)):
                ...
                if (
                    name not in self.cdb.sname2name
                    and raw_name not in self.cdb.sname2name
                ):
                    ...
                    if (
                        ...
                        if (
                            not self.train
                            or not self._train_skip(raw_name)
                            or self.force_train
                        ):
                            if self.DISAMB_EVERYTHING:
                                self.to_disamb.append((list(tkns), raw_name))
                        ...
                        not skip
                        and name in self.cdb.name2cui
                        and len(name) > self.MIN_CONCEPT_LENGTH
                    ):
                        if (
                            not self.train
                            or not self._train_skip(name)
                            or self.force_train
                        ):
                            if self.DISAMB_EVERYTHING:
                                self.to_disamb.append((list(tkns), name))
    ...
    def disambiguate(self, to_disamb):
        ...
        for concept in to_disamb:
            ...
            if self.IS_TRAINER and (
                self.TUI_FILTER is not None or self.CUI_FILTER is not None
            ):
                new_cuis = []
                for cui in cuis:
                    if (
                        self.TUI_FILTER is not None
                        and self.cdb.cui2tui.get(cui, "unk:unk") in self.TUI_FILTER
                    ):
                        new_cuis.append(cui)
            ...
            if len(cuis) > 0 and do_disamb:
                ...
                if self.PREFER_CONCEPTS_WITH is not None:
                    ...
                    noicd = [
                        False
                        if self.PREFER_CONCEPTS_WITH in self.cdb.cui2info.get(cui, {})
                        else True
                        for cui in cuis
                    ]
                    mps[noicd] = 0.5

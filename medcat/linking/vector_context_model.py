...
class ContextModel(object):
    ...
    def get_context_tokens(self, entity, doc, size):
        ...
        tokens_left = [
            ...
            if not tkn._.to_skip
            and not tkn.is_stop
            and not tkn.is_digit
            and not tkn.is_punct
        ]
    ...
    def get_context_vectors(self, entity, doc):
        ...
        for context_type in self.config.linking["context_vector_sizes"].keys():
            ...
            values.extend(
                [
                    self.config.linking["weighted_average_function"](step)
        ...
        if (
            ...
            for context_type in self.config.linking["context_vector_weights"]:
                ...
                if context_type in vectors and context_type in cui_vectors:
                    ...
                    # DEBUG
                    self.log.debug(
                        "Similarity for CUI: {}, Count: {}, Context Type: {:10}, Weight: {:.2f}, Similarity: {:.3f}, S*W: {:.3f}".format(
                            cui,
        ...
        if cuis:  # Maybe none are left after filtering
            ...
            if self.config.linking.get("prefer_primary_name", 0) > 0:
                ...
                for i, cui in enumerate(cuis):
                    if similarities[i] > 0:
                        if self.cdb.name2cuis2status.get(name, {}).get(cui, "") in {
                            "P",
                            "PD",
                        }:
                            ...
                            # DEBUG
                            self.log.debug(
                                "CUI: {}, Name: {}, Old sim: {:.3f}, New sim: {:.3f}".format(
                                    cui, name, old_sim, similarities[i]
                                )
                            )
    ...
    def train(self, cui, entity, doc, negative=False, names=[]):
        ...
        if len(entity) > 0:  # Make sure there is something
            ...
            if negative:
                ...
                for name in names:
                    if self.cdb.name2cuis2status.get(name, {}).get(cui, "") == "P":
                        ...
                        # Debug
                        self.log.debug(
                            "Updating status for CUI: {}, name: {} to <PD>".format(
                                cui, name
                            )
                        )
                        ...
                        self.log.debug(
                            "Updating status for CUI: {}, name: {} to <N>".format(
                                cui, name
                            )
                        )
            if not negative and self.config.linking.get(
            ...
            self.log.warning(
                "The provided entity for cui <{}> was empty, nothing to train".format(
                    cui
                )
            )
    ...
    def train_using_negative_sampling(self, cui):
        ...
        for context_type in self.config.linking["context_vector_sizes"].keys():
            ...
            # Debug
            self.log.debug(
                "Updating CUI: {}, with {} negative words".format(cui, len(inds))
            )

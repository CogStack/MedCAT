...
class Linker(PipeRunner):
    ...
    def _train(self, cui, entity, doc, add_negative=True):
        ...
        if self.train_counter.get(name, 0) > self.config.linking["subsample_after"]:
            if random.random() < 1 / (
                self.train_counter.get(name) - self.config.linking["subsample_after"]
            ):
                self.context_model.train(cui, entity, doc, negative=False)
                if (
                    add_negative
                    and self.config.linking["negative_probability"] >= random.random()
                ):
                ...
                if entity._.link_candidates is not None:
                    if entity._.detected_name is not None:
                        ...
                        if len(cuis) > 0:
                            ...
                            elif len(cuis) == 1 and self.cdb.name2cuis2status[name][
                                cuis[0]
                            ] in {"N", "PD"}:
                            ...
                            if do_disambiguate:
                                (
                                    cui,
                                    context_similarity,
                                ) = self.context_model.disambiguate(
                                ...
                                if self.config.linking["always_calculate_similarity"]:
                                    ...
                                    context_similarity = (
                                        1  # Direct link, no care for similarity
                                    )
                    else:
                    ...
                    if cui and check_filters(cui, self.config.linking["filters"]):
                        ...
                        if (
                            ...
                            >= self.config.linking["similarity_threshold"]
                        ) or (
                            th_type == "dynamic"
                            and context_similarity
                            >= self.cdb.cui2average_confidence[cui]
                            * self.config.linking["similarity_threshold"]
                        ):
                            entity._.cui = cui
    ...
    def _make_pretty_labels(self, doc, style=None):
        ...
        for ent in ents:
            if style == "short":
                ...
                label = "{} | {} | {:.2f}".format(
                    ent._.cui, self.cdb.get_name(ent._.cui), ent._.context_similarity
                )
            else:
                label = "concept"

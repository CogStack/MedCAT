...
class MixingConfig(FakeDict):
    ...
    def save(self, save_path: str) -> None:
        ...
        json_string = jsonpickle.encode(
            {field: getattr(self, field.name) for field in self.fields()})
...
if __name__ == '__main__':
    ...
    print('BaseConfig', bc)
    # print('as dict', asdict(bc))
    print('as dict', bc.dict())
    print('with re', bc.word_skipper, 'and', bc.punct_checker)
    print('\n')
    print('Attempting to get with ["key"]:', bc.general['log_level'])
    print('\n')
    print('Now attempting to SET with ["key"]')
    bc.general['log_level'] = logging.WARN
    print('Set to', bc.general.log_level)
    print('\n')
    print('CORRECT setting')
    bc.general.log_level = logging.INFO
    print('Now at', bc.general.log_level)

    print('Testing MERGING')
    g1: General = General(spacy_model='model_1')
    g2: General = General(spacy_model='/tmp/tm123/model_2')
    print(g1.spacy_model, 'vs', g2.spacy_model)
    # g1.merge_config(asdict(g2))
    g1.merge_config(g2.dict())
    print(g1.spacy_model, 'vs', g2.spacy_model)
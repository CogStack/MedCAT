...
from medcat.utils.meta_cat.ml_utils import (
    predict,
    train_model,
    set_all_seeds,
    eval_model,
...
class MetaCAT(PipeRunner):
    ...
    def prepare_document(self, doc, input_ids, offset_mapping, lowercase):
        ...
        for ent in sorted(ents, key=lambda ent: ent.start_char):
            ...
            if replace_center is not None:
                ...
                tkns = (
                    tkns[:cpos]
                    + self.tokenizer(replace_center)["input_ids"]
                    + tkns[cpos + ln + 1 :]
                )
    ...
    # Override
    def pipe(
        self, stream: Iterable[Doc], *args, **kwargs
        ...
        config = self.config
        id2category_value = {
            v: k for k, v in config.general["category_value2id"].items()
        }
        batch_size_chars = config.general["pipe_batch_size_in_chars"]

        if (
            config.general["device"] == "cpu"
            or config.general["disable_component_lock"]
        ):
            yield from self._set_meta_anns(
                stream, batch_size_chars, config, id2category_value
            )
        else:
    ...
    def _set_meta_anns(self, stream, batch_size_chars, config, id2category_value):
        for docs in self.batch_generator(stream, batch_size_chars):
            try:
                if (
                    not config.general["save_and_reuse_tokens"]
                    or docs[0]._.share_tokens is None
                ):
                    ...
                    for i, doc in enumerate(docs):
                        ent_id2ind, samples = self.prepare_document(
                            doc,
                            input_ids=all_text_processed[i]["input_ids"],
                        ...
                        doc_ind2positions[i] = (
                            ...
                            ent_id2ind,
                        )  # Needed so we know where is what in the big data array
                        data.extend(samples)
                ...
                for i, doc in enumerate(docs):
                    ...
                    for ent in ents:
                        ...
                        if ent._.meta_anns is None:
                            ...
                            ent._.meta_anns[config.general["category_name"]] = {
                                "value": value,
                                "confidence": float(confidence),
                                "name": config.general["category_name"],
                            }

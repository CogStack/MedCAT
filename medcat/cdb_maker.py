        ...
        for csv_path in csv_paths:
            ...
            for row_id, row in enumerate(df[cols].values):
                if row_id % _logging_freq == 0:
                    ...
                    self.log.info(
                        "Current progress: {:.0f}% at {:.3f}s per {} rows".format(
                            (row_id / len(df)) * 100,
                ...
                if not only_existing_cuis or (
                    only_existing_cuis and cui in self.cdb.cui2names
                ):
                    if "ontologies" in col2ind:
                        ontologies = set(
                            [
                                ...
                                .upper()
                                .split(self.cnf_cm["multi_separator"])
                    ...
                    raw_names = [
                        raw_name.strip()
                        for raw_name in row[col2ind["name"]].split(
                            self.cnf_cm["multi_separator"]
                        )
                    ...
                    for raw_name in raw_names:
                        ...
                        if (
                            self.config.cdb_maker.get("remove_parenthesis", 0) > 0
                            and name_status == "P"
                        ):
                            ...
                            if (
                                len(raw_name)
                                >= self.config.cdb_maker["remove_parenthesis"]
                            ):
                                prepare_name(raw_name, self.nlp, names, self.config)
                    ...
                    self.log.debug(
                        "\n\n**** Added\n CUI: {}\n Names: {}\n Ontologies: {}\n Name status: {}\n".format(
                            cui, names, ontologies, name_status
                        )

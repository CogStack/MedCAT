...
class PipeRunner(Pipe):
    ...
    def pipe(
        self, stream: Iterable[Doc], batch_size: int, **kwargs
        ...
        if kwargs.get("parallel", False):
            ...
            for docs in minibatch(stream, size=self.workers):
                ...
                try:
                    tasks = (
                        PipeRunner._delayed(self.__call__, doc, Underscore.get_state())
                        for doc in docs
                    )
    ...
    def _lazy_init_pool(self):
        if PipeRunner._execute is None or self.workers > PipeRunner._execute.n_jobs:
            PipeRunner._execute = Parallel(
                n_jobs=self.workers, timeout=PipeRunner._time_out_in_secs
            )

...
class NER(PipeRunner):
    ...
    def __call__(self, doc):
        ...
        for i in range(len(_doc)):
            ...
            if (
                name
            ):  # There has to be at least something appended to the name to go forward
                for j in range(i + 1, len(_doc)):
                    if (
                        _doc[j].i - _doc[j - 1].i - 1
                    ...
                    for name_version in name_versions:
                        ...
                        if self.config.ner.get("try_reverse_word_order", False):
                            _name_reverse = (
                                name_version + self.config.general["separator"] + name
                            )
                    ...
                    if name_changed:
                        ...
                        if name_reverse in self.cdb.name2cuis:
                            maybe_annotate_name(
                                name_reverse, tkns, doc, self.cdb, self.config
                            )
                    else:
                        break

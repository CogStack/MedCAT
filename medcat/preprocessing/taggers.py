...
def tag_skip_and_punct(doc, config):
    ...
    for token in doc:
        if config.punct_checker.match(token.text) and token.text not in cnf_p['keep_punct']:
            # There can't be punct in a token if it also has text
            token._.is_punct = True
            token._.to_skip = True
        elif not token._.to_skip and config.word_skipper.match(token.lower_):
            # Skip if specific strings
            token._.to_skip = True
        elif cnf_p['skip_stopwords'] and token.is_stop:
            # No need to check if first IF goes through
            token._.to_skip = True

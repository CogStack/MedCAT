        ...
        d.append(
            {
                "text": doc.text,
                "entities": [
                    (e.start_char, e.end_char, cdb.tui2name[cdb.cui2tui[e.label_]])
                    for e in doc._.ents
                ],
            }
        )
    ...
    for span in list(doc.ents):
        ...
        ent = {
            ...
            "bg": "rgb(74, 154, 239, {})".format(
                span._.context_similarity * span._.context_similarity + 0.12
            ),
...
def prepare_name(cat, name, version="CLEAN"):
    ...
    if version.lower() == "raw":
        ...
        tokens = [
            t.lower_
            for t in sc_name
            if not t._.is_punct and not (t._.to_skip and not t.is_stop)
        ]

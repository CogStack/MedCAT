...
def are_anns_same(ann, ann2, meta_names=[], require_double_inner=True):
    if (
        ann["cui"] == ann2["cui"]
        and ann["correct"] == ann2["correct"]
        and ann["deleted"] == ann2["deleted"]
        and ann["alternative"] == ann2["alternative"]
        and ann["killed"] == ann2["killed"]
        and ann["manually_created"] == ann2["manually_created"]
        and ann["validated"] == ann2["validated"]
    ):
    ...
    if ann_stats:
        ...
        for i in range(len(_ann_stats)):
            if _ann_stats[i, 0] == _ann_stats[i, 1]:
                t += 1
        print(
            "Overall given the parameters (scores here can be strange, be sure to know what they mean)"
        )
        ...
        for did, doc in enumerate(project["documents"]):
            ...
            if not doc.get("_verified", False) or ignore_already_done or only_saved:
                ...
                for id, p_ann in enumerate(p_anns_norm):
                    if (only_start and p_ann[0] not in t_anns_start) or (
                        not only_start and p_ann not in t_anns_norm
                    ):
                        ann = s_doc.ents[id]
                        if not only_saved:
                            ...
                            cui = ann._.cui
                            b = (
                                text[max(0, start - cntx_size) : start]
                                .replace("\n", " ")
                                .replace("\r", " ")
                            )
                            m = text[start:end].replace("\n", " ").replace("\r", " ")
                            e = (
                                text[end : min(len(text), end + cntx_size)]
                                .replace("\n", " ")
                                .replace("\r", " ")
                            )
                            ...
                            print(
                                cui,
                                " | ",
                                cat.cdb.cui2pretty_name.get(cui, ""),
                                " | ",
                                cat.cdb.cui2tui.get(cui, ""),
                                " | ",
                                ann.start_char,
                            )
                            print(ann._.acc)
                            d = str(
                                input(
                                    "###Add as (or empty for skip): 1-Correct, 2-Incorrect, s-save: "
                                )
                            )
                ...
                for id, t_ann in enumerate(t_anns_norm):
                    ...
                    if (
                        (not only_saved and not only_start and t_ann not in p_anns_norm)
                        or (
                            not only_saved
                            and only_start
                            and t_ann[0] not in p_anns_start
                        )
                        or (only_saved and ann.get("_saved", False))
                    ):
                        # Check is it correct
                        if (
                            not ann.get("_verified", False)
                            or ignore_already_done
                            or (only_saved and ann.get("_saved", False))
                        ):
                            print(
                            ...
                            b = (
                                text[max(0, ann["start"] - cntx_size) : ann["start"]]
                                .replace("\n", " ")
                                .replace("\r", " ")
                            )
                            m = (
                                text[ann["start"] : ann["end"]]
                                .replace("\n", " ")
                                .replace("\r", " ")
                            )
                            e = (
                                text[
                                    ann["end"] : min(len(text), ann["end"] + cntx_size)
                                ]
                                .replace("\n", " ")
                                .replace("\r", " ")
                            )
                            ...
                            print(
                                ann["cui"],
                                " | ",
                                cat.cdb.cui2pretty_name.get(ann["cui"], ""),
                                " | ",
                                ann["start"],
                            )
                            ...
                            d = str(
                                input(
                                    "###Change to (or empty for skip): 1-Correct, 2-Incorrect, d-delete, s-save: "
                                )
                            )
...
def consolidate_double_annotations(
    data_path,
    out_path,
    require_double=True,
    require_double_inner=False,
    meta_anns_to_match=[],
):
    ...
    for project in data["projects"]:
        ...
        if id not in projects_done:
                ...
                if has_double:
                    for document in documents:
                        ...
                        if document2 is not None:
                            new_document = get_same_anns(
                                document,
                                document2,
                                require_double_inner=require_double_inner,
                                ann_stats=ann_stats_project,
                                meta_names=meta_anns_to_match,
                            )
            ...
            if ann_stats_project:
                ...
                print(
                    "** Project '{}' did not have double annotations\n\n".format(
                        project["name"]
                    )
                )
...
def validate_ner_data(
    data_path, cdb, cntx_size=70, status_only=False, ignore_if_already_done=False
):
    ...
    print(
        "If you want to [q]uit write  q  your progress will be saved to the json and you can continue later"
    )
            ...
            for ann in document["annotations"]:
                ...
                if cui in cui2status:
                    if name in cui2status[cui]:
                        cui2status[cui][name][status] = (
                            cui2status[cui][name].get(status, 0) + 1
                        )
                    else:
        ...
        for project in data["projects"]:
            for document in project["documents"]:
                ...
                for ann in document["annotations"]:
                    ...
                    if (
                        "manul_verification_mention" not in ann
                        or ignore_if_already_done
                    ):
                        if ann["correct"]:
                            status = "Correct"
                        else:
        ...
        for project in data["projects"]:
            for document in project["documents"]:
                ...
                for ann in document["annotations"]:
                    ...
                    if (
                        "manual_verification_status" not in ann
                        or ignore_if_already_done
                    ):
                        if ann["correct"]:
                            status = "correct"
                        ...
                        if len(cui2status[cui][name]) > 1:
                            ...
                            print(
                                "CURRENT ANNOTATION: {} - {}".format(
                                    cui, cdb.cui2pretty_name.get(cui, "unk")
                                )
                            )
                            print("ANNS TOTAL        :")
                            ...
                            d = str(
                                input(
                                    "###Change to ([q]uit/[s]kip/[c]orrect/[i]ncorrect/[t]erminate): "
                                )
                            )
...
def prepare_from_json_hf(
    ...
    p_data = prepare_from_json_chars(
        data,
        cntx_left=cntx_left,
        cntx_right=cntx_right,
        tokenizer=tokenizer,
...
def prepare_from_json_chars(
    data, cntx_left, cntx_right, tokenizer, cui_filter=None, replace_center=None
):
...
def make_mc_train_test(data, cdb, seed=17, test_size=0.2):
        ...
        for document in project["documents"]:
            ...
            for ann in doc_annotations:
                if (cui_filter is None and tui_filter is None) or (
                    cui_filter is not None and ann["cui"] in cui_filter
                ):
                    if ann["cui"] in cnts:
                        cnts[ann["cui"]] += 1
                    else:
    ...
    for i_project in np.random.permutation(np.arange(0, len(data["projects"]))):
        ...
        for i_document in np.random.permutation(
            np.arange(0, len(project["documents"]))
        ):
            ...
            for ann in doc_annotations:
                if (
                    (cui_filter is None and tui_filter is None)
                    or (cui_filter is not None and ann["cui"] in cui_filter)
                    or (
                        tui_filter is not None
                        and cdb.cui2tui.get(ann["cui"], "unk") in tui_filter
                    )
                ):
                    if ann["cui"] in _cnts:
                        _cnts[ann["cui"]] += 1

    ...
    def to_concept_df(self):
        ...
        for i, snomed_release in enumerate(snomed_releases):
            ...
            int_terms = parse_file(
                f"{contents_path}/sct2_Concept_Snapshot_{snomed_v}_{snomed_release}.txt"
            )
            active_terms = int_terms[int_terms.active == "1"]
            del int_terms

            int_desc = parse_file(
                f"{contents_path}/sct2_Description_Snapshot-en_{snomed_v}_{snomed_release}.txt"
            )
    ...
    def list_all_relationships(self):
        ...
        for i, snomed_release in enumerate(snomed_releases):
            ...
            int_relat = parse_file(
                f"{contents_path}/sct2_Relationship_Snapshot_{snomed_v}_{snomed_release}.txt"
            )
        ...
        for i, snomed_release in enumerate(snomed_releases):
            ...
            for f in os.listdir(refset_terminology):
                ...
                if m:
                    snomed_v = m.group(1)
            mappings = parse_file(
                f"{refset_terminology}/der2_iisssccRefset_ExtendedMapSnapshot_{snomed_v}_{snomed_release}.txt"
            )
        ...
        for i, snomed_release in enumerate(snomed_releases):
            ...
            if snomed_v == "":
                raise FileNotFoundError(
                    "This SNOMED release does not contain OPCS mapping files"
                )
            mappings = parse_file(
                f"{refset_terminology}/der2_iisssciRefset_ExtendedMapSnapshot_{snomed_v}_{snomed_release}.txt"
            )
            mappings = mappings[mappings.active == "1"]
            icd_mappings = mappings.sort_values(
                by=["referencedComponentId", "mapPriority", "mapGroup"]

...
from transformers import BertModel, AutoConfig, ModernBertModel, ModernBertConfig
...
class ModernBertForMetaAnnotation(nn.Module):
    _keys_to_ignore_on_load_unexpected: List[str] = [r"pooler"]  # type: ignore

    def __init__(self, config):
        super(ModernBertForMetaAnnotation, self).__init__()
        _modernbertconfig = AutoConfig.from_pretrained(config.model.model_variant,num_hidden_layers=config.model['num_layers'])
        if config.model['input_size'] != _modernbertconfig.hidden_size:
            logger.warning("Input size for %s model should be %d, provided input size is %d. Input size changed to %d",config.model.model_variant,_modernbertconfig.hidden_size,config.model['input_size'],_modernbertconfig.hidden_size)
        modernbert_model = ModernBertModel.from_pretrained(config.model.model_variant, config=_modernbertconfig)
        self.config = config
        self.config.use_return_dict = False
        ...
        self.relu = nn.ReLU()
        # dense layer 1
        self.fc1 = nn.Linear(_modernbertconfig.hidden_size*2, config.model.hidden_size)
        # dense layer 2
        self.fc2 = nn.Linear(config.model.hidden_size, hidden_size_2)
        # dense layer 3
        self.fc3 = nn.Linear(hidden_size_2, hidden_size_2)

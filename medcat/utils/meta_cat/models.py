...
class LSTM(nn.Module):
    def __init__(self, embeddings, config):
        ...
        self.embeddings = nn.Embedding(
            vocab_size, embedding_size, padding_idx=config.model["padding_idx"]
        )
    ...
    def forward(
        self, input_ids, center_positions, attention_mask=None, ignore_cpos=False
    ):
        x = input_ids
        # Get the mask from x
        ...
        x = nn.utils.rnn.pack_padded_sequence(
            x, mask.sum(1).int().view(-1).cpu(), batch_first=True, enforce_sorted=False
        )
        ...
        if ignore_cpos:
            ...
            x = (
                x[-1, :, :, :]
                .permute(1, 2, 0)
                .reshape(-1, self.config.model["hidden_size"])
            )
        else:

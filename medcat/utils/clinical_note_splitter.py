...
def normalize_date(date, id, start, end):
    ...
    if "-" in date:
        ...
        logging.warning(
            "Unsuported date format: {} for id: {} with start: {}, end: {}".format(
                date, id, start, end
            )
        )
        return None
        ...
        if start == 0:
            ...
            if "entered on -" in note_text.lower():
                if len(regex.findall(r"entered on -", note_text)) > 1:
                    logging.warning(
                        "Possible problems for span with start: {} and end: {} for note with id: {}".format(
                            start, end, id
                        )
                    )
                split_note.append(
                    {
                        ...
                        "end": end,
                        "text": note_text,
                        "date": normalize_date(previous_date, id, start, end),
                    }
                )
                start = end
                previous_date = date.captures()[0]
        ...
        split_note.append(
            {
                ...
                "end": len(text),
                "text": text[start:],
                "date": normalize_date(previous_date, id, start, len(text)),
            }
        )
    else:
        logging.warning(
            "No date/entered-on detected for id: {} wth start: {}, end: {} and text:\n{}...".format(
                id, start, end, text[0:300]
            )
        )

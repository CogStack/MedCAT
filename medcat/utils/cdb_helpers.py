...
def mrconso_to_csv(
    mrconso_path, column_names=None, sep="|", lng="ENG", output_path=None, **kwargs
):
...
def umls_to_snomed_name_extension(
    mrconso_path,
    snomed_codes,
    column_names=None,
    sep="|",
    ...
    **kwargs
    ...
    umls2snomed = {
        cui: snomed for cui, snomed in umls2snomed.items() if cui not in cuis_to_remove
    }
...
def snomed_source_to_csv(
    snomed_term_paths=[],
    snomed_desc_paths=[],
    sep="\t",
    output_path=None,
    output_path_type_names=None,
    strip_fqn=True,
    na_filter=False,
    **kwargs
):
    ...
    # Process terms
    snomed_terms = [
        pd.read_csv(path, sep=sep, dtype=str, **kwargs) for path in snomed_term_paths
    ]
    ...
    snomed_descs = [
        pd.read_csv(path, sep=sep, dtype=str, na_filter=na_filter, **kwargs)
        for path in snomed_desc_paths
    ]
    ...
    snomed_cdb_df = snomed_cdb_df.rename(
        columns={"conceptId": "cui", "term": "name", "typeId": "name_status"}
    )
    # Ontology is always SNOMED
    snomed_cdb_df["ontologies"] = ["SNOMED"] * len(snomed_cdb_df)
    # Take primary names
    snomed_cdb_df["name_status"] = [
        "P" if name_status == "900000000000003001" else "A"
        for name_status in snomed_cdb_df.name_status.values
    ]

    # Get type names and IDs, there is no real way to do this, so I'll invent a type ID
    cui2type_name = {
        cui: name[name.rfind("(") + 1 : name.rfind(")")]
        for cui, name in snomed_cdb_df[snomed_cdb_df["name_status"] == "P"][
            ["cui", "name"]
        ].values
        if name.endswith(")")
    }
    ...
    type_name2id = {
        type_name: "T-{}".format(id)
        for id, type_name in enumerate(sorted(set(cui2type_name.values())))
    }

    # Add stripped FQNs if necessary, they will be appended at the end of the dataframe
    if strip_fqn:
        fqn_stripped = snomed_cdb_df[
            [
                True if name_status == "P" and name.endswith(")") else False
                for name, name_status in snomed_cdb_df[["name", "name_status"]].values
            ]
        ]
        fqn_stripped["name"] = [
            name[0 : name.rfind("(")].strip() for name in fqn_stripped["name"].values
        ]
        snomed_cdb_df = pd.concat([snomed_cdb_df, fqn_stripped])

    # Add type_ids column to the output df
    snomed_cdb_df["type_ids"] = [
        type_name2id.get(cui2type_name.get(cui, "unk"), "unk")
        for cui in snomed_cdb_df.cui
    ]

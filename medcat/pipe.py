...
class Pipe(object):
    ...
    def __init__(self, tokenizer: Tokenizer, config: Config) -> None:
        if config.pre_load.preprocessing_stopwords is not None:
            lang = os.path.basename(config.pre_load.spacy_model).split('_', 1)[0]
            cls = spacy.util.get_lang_class(lang)
            cls.Defaults.stop_words = set(config.pre_load.preprocessing_stopwords)
        try:
            self._nlp = self._init_nlp(config)
        except Exception as e:
            if config.pre_load.spacy_model == DEFAULT_SPACY_MODEL:
                raise e
            logger.warning("Could not load spacy model from '%s'. "
                           ...
                           config.pre_load.spacy_model, exc_info=e)
            ...
            config.pre_load.spacy_model = DEFAULT_SPACY_MODEL
            self._nlp = self._init_nlp(config)
        self._nlp.tokenizer = tokenizer(self._nlp, config)
        # Set max document length
        self._nlp.max_length = config.pre_load.max_document_length
        self.config = config
        # Set log level
        logger.setLevel(self.config.pre_load.log_level)

    def _init_nlp(selef, config: Config) -> Language:
        return spacy.load(config.pre_load.spacy_model, disable=config.pre_load.spacy_disabled_components)

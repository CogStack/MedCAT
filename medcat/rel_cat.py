...
class RelCAT(PipeRunner):
    ...
    def create_test_train_datasets(self, data, split_sets=False):
        ...
        if split_sets:
            ...
            test_data_label_names = [rec[4]
                                     for rec in test_data["output_relations"]]
        ...
        for k, v in data.items():
            if k != "output_relations":
                train_data[k] = []
                test_data[k] = []
        entity_tags_ = self.config.general.annotation_schema_tag_ids

        idx_ = 0
        for rec in train_data["output_relations"]:
            index_start_e1 = rec[0].index(entity_tags_[0])
            index_end_e2 = rec[0].index(entity_tags_[3])

            index_start_e1 = max(0, index_start_e1 - self.config.general.cntx_left)
            index_end_e2 = min(len(rec[0]), index_end_e2 + self.config.general.cntx_right)

            train_data["output_relations"][idx_][0] = rec[0][index_start_e1:index_end_e2 + 1]
            idx_ += 1
    ...
    def train(self, export_data_path="", train_csv_path="", test_csv_path="", checkpoint_path="./"):
        ...
        if train_csv_path != "":
            if test_csv_path != "":
                train_rel_data.dataset, _ = self.create_test_train_datasets(
                    train_rel_data.create_base_relations_from_csv(train_csv_path), split_sets=False)
                test_rel_data.dataset, _ = self.create_test_train_datasets(
                    train_rel_data.create_base_relations_from_csv(test_csv_path), split_sets=False)
            else:
                train_rel_data.dataset, test_rel_data.dataset = self.create_test_train_datasets(
            ...
            for i, data in enumerate(predict_dataloader):
                with torch.no_grad():
                    ...
                    for i, pred_rel_logits in enumerate(pred_classification_logits):
                        ...
                        doc._.relations.append({"relation": self.config.general.idx2labels[str(predicted_label_id)],
